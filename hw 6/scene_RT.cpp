...
mObject_RT::operator ()(ray_RT ray, ray_RT& camPos) //vector_RT<RT_default_type>& camera)
{
	auto res = std::move(this->ray_hit(camPos, ray)) ;
...
operator <<(std::ostream& os, const mObject_RT& obj)
{
	...
	return(os) ;
																		...
																		// class mScene_RT
mColor
mScene_RT::operator()(ray_RT ray, ray_RT& camPos) // vector_RT<RT_default_type>& camera)
{
	...
	for (auto it = cbegin(_base) ; it != cend(_base) ; ++it) {
		current = std::move(it->ray_hit(camPos, ray)) ;

...
cl_seqLightsRT::cl_seqLightsRT(cl_SceneDescr* sd)
{
	...
	_base.shrink_to_fit() ;
																		...
																		// eoc lights
																		// classes for materials, etc
cl_seqTexturesRT::cl_seqTexturesRT(cl_SceneDescr* sd)
{
	if (sd->activate())   cout << " & materials" ;
	else				  throw std::runtime_error("___ scene: cannot activate the descriptor") ;
	...
	: _objects{objs}, _lights{lights}, _clrb{}, _shadowBias{}, _set_concurrency{}, _textures{tex}
{
	...
	if (_shadowBias == 0)  _shadowBias = static_cast<RT_default_type>(0.01) ; // .002) ;

	scene_fine_tune() ;		// the Scene is set - fine tune it: like veritices normals(, etc)

	cout << endl << endl << ": # of objects: " << _objects->data().size() ;
	int i = 0 ;
	for (auto dr : _objects->data()) {
...
{
	for (auto dr : _objects->data()) { dr.obj_fine_tune(smooth_shading(dr)) ; }
...
mScene_RT::set_concurrency(cl_SceneDescr * sd, unsigned int W, unsigned int H)
{
	...
	{ // prepare the slices
		...
		if (rn == 0)	rn = (H / 64) ; // # of horizontal cuts: 64 pixels in a cut 

		size_t   cn{number_from_string<size_t>
				(*(sd->begin_data(sd->data("\"concurrency\"", "\"#_of_vertical_cuts\""), 1)))
		} ;
		if (cn == 0)	cn = (W / 64) ; // # of vertical cuts: 64 pixels in a cut 
...
mScene_RT::ray_hit(size_t ind_iter, const ray_RT& orig, const ray_RT& ray, ray_HP& hP)
{
	...
	auto		tr_next { (_objects->data())[ind_iter].obj_begin() };
	auto		tr_end  { (_objects->data())[ind_iter].obj_end() };
	decltype(tr_next)	ind ;

	if ((_objects->data())[ind_iter].box_hit(orig, ray)) {
...
bool
mScene_RT::in_shadow(const mAtom_RT& tr, const ray_RT& hP, const ray_RT& ld, double l_dist) // , mAlbedo& shalb)
{
	ray_RT::value_type cdist{} ;	// compare against distance to light l_dist
	ray_HP			   whit ;		// ...

	for (auto obj = cbegin(_objects->data()), obj_e = cend(_objects->data()) ; obj != obj_e ; ++obj) {
		for (auto tr = obj->obj_begin(), tr_e = obj->obj_end() ; tr != tr_e ; ++tr) {
			if (tr->_ray_hit(hP, ld, cdist, whit)) {
				if (cdist < l_dist) { 
					/* { shalb = albedo(*obj) ; return true ; } */	
					if (dotPR(ld, whit._hp) > 0)	return true ;
				}
			}
		}
	}
...
mScene_RT::shadow_color(const mAtom_RT& tr, ray_HP& hP, const cl_objectRT& ob)
{
	...
	for (auto li = _lights->begin_lights(), li_end = _lights->end_lights() ; li != li_end ; ++li) {
		...
		if (!(in_shadow(tr, hP._hp, lightDir, sr))) { // , shalb)))	{
	...
	// if (final_color == mAlbedo{})    final_color = mAlbedo{1.f, 0.f, 0.f} ;
	return final_color ;
...
mScene_RT::operator()(ray_RT ray, const ray_RT& camPos)
{
	...
	for (int depth = 0 ; depth < Scene_MAX_Depth ; depth++) { // recursion for other than diffuse textures
		...
		size_t	nfound{(_objects->data()).size()} ; size_t  chosen_object{nfound} ;
		...
		if (reflective((_objects->data())[chosen_object])) {
			...
			ray = reflected_ray(ray, (_objects->data())[chosen_object], *(chosen.first)) ;
			...
			return shadow_color(*(chosen.first), hP, ((_objects->data())[chosen_object])) ;
		}
	}
	return shadow_color(*atom_first, hP_first, (_objects->data())[obj_first]) ;
	// return(mColor{}) ;
} // mScene operator()

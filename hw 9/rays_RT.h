...
render_imSlcV2(Funct func, Conc co) // , const std::vector<slcRays>& iSlices)
{
	size_t	nTh{co._num_of_threads} ; // std::thread::hardware_concurrency() } ;
	// nTh = 200 ;
	
	auto&	iSlices = co._slices ;
	auto&	slcAssigned = co._slc_assign ;

	cout << endl << endl << "> camera #" << func.camera_id() << ": " << iSlices.size() 
						<< " slices; threads used: " << nTh << endl ;
	assert(nTh <= iSlices.size()) ;
...
{
	// atomChecks = boxMisses = boxChecks = 0 ; // for statistics

	size_t	nTh{co._num_of_threads} ; // std::thread::hardware_concurrency() } ;

	{
		auto	n_slc = co._slices.size() ;
		cout << endl << endl << "> camera #" << func.camera_id() << ": " << n_slc
			<< " slices; threads used: " << nTh << endl ;
		assert(nTh <= n_slc) ;
	}
